...
void GetPhysicalMemoryInformation(SMBIOSPhysicalMemory* curStruct, RawSMBIOSData* rawdata) {
    ...
    if ((int)curStruct->Size == 0) return;

    if (rawdata->SMBIOSMajorVersion < 2 ||
        (rawdata->SMBIOSMajorVersion == 2 && rawdata->SMBIOSMinorVersion < 1)) {
        return;
    }
    ...
    PhysicalMemoryInformation.push_back(
        "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");  // just a seperator for
                                                              // visibility
...
void GetBaseBoardInformation(SMBIOSBaseBoard* curStruct, RawSMBIOSData* rawdata) {
    std::vector<std::string> strings = ConvertSMBIOSString(curStruct);
    BaseBoardSerial =
        strings[curStruct->SerialNumber] +
        strings[curStruct->Product];  // different products could have same serial number,
                                      // just a better way to deal with this.
